...
def Activate_DNS(obj):
	if button_dns.get_label() == "Activate":
		...
		if dns != "" and ";" not in dns:
			dns_split = dns.split(".")
			if dns_split[0].isdigit():
				if dns_split[1].isdigit():
					if dns_split[2].isdigit():
						if dns_split[3].isdigit():
							status = subprocess.Popen(
								"/usr/bin/nordvpn set dns " + dns_split[0] + "." + dns_split[1] + "." + dns_split[2] +
								"." + dns_split[3],
								shell=True,
								stdout=subprocess.PIPE,
								stderr=subprocess.PIPE, universal_newlines=True)
...
# SETTINGS
box6aa = Gtk.Box(spacing=6)
box6aa.show()
box1.pack_start(box6aa, False, False, 0)
...
box6bb = Gtk.Box(spacing=6)
box6bb.show()
box1.pack_start(box6bb, False, False, 0)
box6b = Gtk.Box(spacing=6)
box6b.show()
box1.pack_start(box6b, False, False, 0)
box6cc = Gtk.Box(spacing=6)
box6cc.show()
box1.pack_start(box6cc, False, False, 0)

# MESHNET
box6ccc = Gtk.Box(spacing=6)
box1.pack_start(box6ccc, False, False, 0)
...
# STATUSBAR EOP
box7a = Gtk.Box(spacing=6)
box7a.show()
box1.pack_start(box7a, True, True, 0)
...
# label_empty.show()
box6aa.pack_start(label_empty, False, False, 0)
...
label_leeg = Gtk.Label()
label_leeg.show()
box6bb.pack_start(label_leeg, False, False, 0)
label_tpl = Gtk.Label()
label_tpl.show()
...
label_mes_status.set_size_request(120, -1)
label_mes_status.set_text("Disabled")
