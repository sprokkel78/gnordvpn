...
def Get_File_List():
	...
	if MES == 1 and internet == 1:
		...
		txtp = "\n"
		status = subprocess.Popen("/usr/bin/nordvpn fileshare list", shell=True, stdout=subprocess.PIPE,
					stderr=subprocess.PIPE, universal_newlines=True)
		rcstat = status.wait()
		statout, staterr = status.communicate()
		statout = statout.strip('\n')
...
label_tpl_status.set_size_request(80, -1)
...
label_mes_status.set_size_request(80, -1)
...
label_fss = Gtk.Label()
label_fss.set_size_request(80, -1)
box6d.pack_start(label_fss, False, False, 0)
label_fsss = Gtk.Label()
label_fsss.set_size_request(32, -1)
